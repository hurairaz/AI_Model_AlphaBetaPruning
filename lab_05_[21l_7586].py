# -*- coding: utf-8 -*-
"""Lab#05 [21L-7586].ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1J-KQoEqthemWe_x-BLlRYKwf8iIomurs
"""
...
def evaluate_node(node):
    return node.remaining_budget * (node.investment_option.expected_return - node.investment_option.risk)

def generate_possible_moves(root):
    root.value = evaluate_node(root)
    childCount = rand() % 4

    for _ in range(childCount):
        profitLossDecision = rand() % 3
        profitLossFactor = random.uniform(0.05, 0.15)
        investment_option = InvestmentOption(name='Stocks', expected_return=0.1, risk=0.05)

        if profitLossDecision == 0:
            investment_option.expected_return += profitLossFactor
        elif profitLossDecision == 1:
            investment_option.risk += profitLossFactor

        childNode = Node(investment_option, 3000, 0.25, 3)
        root.children.append(childNode)
        generate_possible_moves(childNode)

    return root

def generate_possible_moves_wrapper(root):
    return generate_possible_moves(root)

def is_terminal(node):
  return len(node.children) == 0

def against_rules(node):
    return node.risk_tolerance < node.investment_option.risk
...
def get_best_move(options, initial_budget, risk_tolerance, investment_horizon):
    ...
    for option in options:
        ...
        generate_possible_moves_wrapper(root)
        roots.append(root)
...
def main():
    ...
    # Plot the game trees for each option
    for i, root in enumerate(roots):
        plot_tree(root, title=f"Game Tree for {options[i].name}")

    # Print the recommended portfolio allocation and the max profit achievable
